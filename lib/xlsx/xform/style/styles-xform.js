...
const {encodeStyle} = require('../../../utils/style-fast-serialize');
 ...
 *
 * - WEAK_MAP: Original behavior using WeakMap (caches by object reference)
 * - JSON_MAP: Uses JSON.stringify for cache keys (caches by value)
 * - FAST_MAP: Uses custom fast serialization for cache keys (fastest)
 * - NO_CACHE: No caching (useful for specific scenarios)
 *
 * @see https://github.com/exceljs/exceljs/pull/2867
 */
const StyleCacheMode = {
  WEAK_MAP: 'weak_map',
  JSON_MAP: 'json_map',
  FAST_MAP: 'fast_map',
  NO_CACHE: 'no_cache',
...
class StylesXform extends BaseXform {
  /**
   * @param {boolean|Object} initialise - If true or object, initialize as style manager
   * @param {string} [initialise.cacheMode] - Style caching mode (weak_map, json_map, fast_map, no_cache)
   */
  constructor(initialise) {
    ...
    // Determine cache mode from options
    if (typeof initialise === 'object' && initialise !== null) {
      this.cacheMode = initialise.cacheMode || StyleCacheMode.WEAK_MAP;
      this.init();
    } else if (initialise) {
      // StylesXform also acts as style manager and is used to build up styles-model during worksheet processing
      this.cacheMode = StyleCacheMode.WEAK_MAP;
      this.init();
    }
  }
  ...
  init() {
    ...
    switch (this.cacheMode) {
      case StyleCacheMode.JSON_MAP:
      case StyleCacheMode.FAST_MAP:
        this.styleCache = new Map();
        break;
      case StyleCacheMode.NO_CACHE:
        this.styleCache = null;
        break;
      case StyleCacheMode.WEAK_MAP:
      default:
  ...
  addStyleModel(model, cellType) {
    ...
    // Store in cache based on mode
    switch (this.cacheMode) {
      case StyleCacheMode.FAST_MAP:
      case StyleCacheMode.JSON_MAP:
        this.styleCache.set(cacheKey, styleId);
        break;
      case StyleCacheMode.NO_CACHE:
        // No caching
        break;
...
StylesXform.StyleCacheMode = StyleCacheMode;
