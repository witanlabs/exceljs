...
class Worksheet {
  ...
  _mergeCellsInternal(dimensions, ignoreStyle) {
    // Get master cell and check if already merged
    const master = this.getCell(dimensions.top, dimensions.left);
    if (master.isMerged) {
      throw new Error('Cannot merge already merged cells');
    }

    // Apply merge, checking each cell for existing merges
...
Please leave feedback at https://github.com/exceljs/exceljs/discussions/2575`
  ...
  set model(value) {
    ...
    this.tables = value.tables.reduce((tables, table) => {
      ...
      // Use !table.headerRow to catch both undefined and false (safety net for parser edge cases)
      ...
      // Create table with isLoading flag - this populates rows from worksheet cells
      // instead of overwriting cells with empty table data
      const t = new Table(this, table, {isLoading: true});
      tables[table.name] = t;
